...
public:
  ...
  // Separate connection used by the serializer so a flush can run while
  // the streaming cursor's read transaction is still open on `conn_`.
  // pqxx forbids nesting transactions on a single connection.
  pqxx::connection& writer() { return writer_conn_; }

private:
  pqxx::connection conn_;
  pqxx::connection writer_conn_;
};
